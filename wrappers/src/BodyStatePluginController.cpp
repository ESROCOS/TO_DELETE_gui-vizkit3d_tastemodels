...
BodyStatePluginController::BodyStatePluginController(vizkit3d::Vizkit3DWidget& widget, YAML::Node& config)
: m_pPlugin(NULL)
{
    assert(config["type"] && !config["type"].as<std::string>().compare("BodyStateVisualization"));
    
    // Plugin type and name
    setType("BodyStateVisualization");
    setNameFromConfig(config);
    setLabel(getName() + " (" + getType() + ")");

    // Create and add plugin
    m_pPlugin = dynamic_cast<vizkit3d::BodyStateVisualization*>(
            widget.loadPlugin("base", "BodyStateVisualization"));
    if (NULL == m_pPlugin)
    {
        ...
        else if (!key.compare("modelPath"))
        {
            if (getStringParam(strVal, config, key, getLabel()))
            {
                ...
                catch (const std::exception& e)
                {
                    ...
                    msg << "vizkit-taste: error loading model " << strVal << 
                        "in BodyStateVisualization plugin: " << e.what();
                    printMsg(msg.str());
                }
            }
        }
...
BodyStatePluginController::~BodyStatePluginController()
{
}

void BodyStatePluginController::updateBodyState(const base::samples::BodyState& state)
{
    assert(NULL != m_pPlugin);
    m_pPlugin->updateBodyState(state);
}
